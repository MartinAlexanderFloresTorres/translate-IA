...
export default function Header() {
  ...
  const handleModelChange = (selected: { value: string } | null) => {
    if (selected) {
      ...
      toast.success(`Modelo seleccionado: ${selected.value}`);
    } else {
      toast.dismiss();
    }
  };
    ...
    <section className="w-full pt-20 pb-8 px-10 header">
      <div className="container mx-auto flex flex-col gap-10 justify-between min-h-[440px]">
        <div className="flex-1">
          <h1 className="text-6xl sm:text-7xl font-extrabold tracking-tighter md:text-8xl text-[#ffe32a] drop-shadow-xl text-center">Traductor de idiomas</h1>
          <p className="max-w-[700px] font-semibold drop-shadow-xl mx-auto mt-6 text-xl sm:text-2xl text-center text-white">¡Traduce cualquier texto a tu idioma preferido 🌍!</p>
          <div className="max-w-[700px] mx-auto flex gap-4 items-center justify-center mt-10 flex-col">
            <input type="text" className={`w-full px-4 py-2 text-gray-600 font-semibold bg-white rounded-md outline-sky-400`} placeholder="Ingresa tu api key de https://platform.openai.com/api-keys" onChange={({ target }) => debounced(target.value)} defaultValue={openAIKey} />
            <Select
        ...
        <div className="flex gap-4 items-center justify-center w-full">
          <a className="flex items-center gap-2 bg-white px-3 py-1 rounded-full" href="https://sdk.vercel.ai" target="_blank" rel="noreferrer noopener">
          ...
          <a className="flex items-center gap-2 bg-white px-3 py-1 rounded-full" href="https://github.com/MartinAlexanderFloresTorres" target="_blank" rel="noreferrer noopener">
